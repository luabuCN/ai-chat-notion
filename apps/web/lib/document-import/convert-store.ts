/**
 * 全局文档导入任务 store（Zustand 单例）。
 *
 * 状态在客户端 JS 上下文中常驻，切换路由不销毁，
...
export function isConvertTaskPipelineBusy(
  ...
  return isImportPipelineBusy(task);
}

/** 是否存在正在解析/保存中的导入任务（用于禁止并发上传） */
export function isDocumentImportBusy(): boolean {
  return Object.values(useConvertTaskStore.getState().tasks).some(
    isImportPipelineBusy
  );
}

/** React：任一文档处于导入或保存中时返回 true */
export function useDocumentImportBusy(): boolean {
  return useConvertTaskStore((s) =>
    Object.values(s.tasks).some(isImportPipelineBusy)
  );
}

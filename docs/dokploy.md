...
3. **Compose type**：选 **Docker Compose**（需要 `build`），不要选 Stack/Swarm。
...
访问 `http://127.0.0.1:3000`（可通过 `WEB_PORT` 改宿主机端口）。
...
- **web**：Dockerfile `apps/web/Dockerfile`，端口 `3000`；构建参数需在 Dokploy 构建配置中传入 `NEXT_PUBLIC_*` 与 `POSTGRES_URL`、`AUTH_SECRET`。
